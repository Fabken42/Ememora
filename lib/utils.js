...
export async function fetchWithTokenRetry(url, options, currentToken, refreshTokenFunction) {
  ...
  // Só tenta renovar se for erro de autenticação (401)
  if (response.status === 401) {
    let data
    try {
      data = await response.clone().json() // clone() para não consumir o stream
    } catch {
      data = {}
    }

    // Checa se o backend realmente indicou expiração do token
    if (data?.code === 'TOKEN_EXPIRED' || data?.error === 'Token expirado') {
      console.warn('Token expirado, tentando renovar...')

      try {
        const newToken = await refreshTokenFunction()

        // Nova tentativa com o token renovado
